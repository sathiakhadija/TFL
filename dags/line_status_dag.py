...
def _ingest_line_status(**context):
    ...
    try:
        ...
        row_count = ingest_line_status(conn, client)
    ...
    try:
        ...
        if not result["valid"]:
            raise AirflowException(
                f"line_status validation failed: {result['checks']} "
...
with DAG(
    dag_id="line_status_ingestion",
    schedule="*/15 * * * *",
    start_date=datetime(2024, 1, 1),
    catchup=False,
    max_active_runs=1,
    default_args=default_args,
    tags=["tfl", "ingestion", "line-status"],
) as dag:
    ...
    ingest = PythonOperator(
        ...
        python_callable=_ingest_line_status,
    )
    ...
    trigger_dbt = BashOperator(
        ...
        bash_command=(
            "dbt run "
            "--select staging.stg_line_status marts.mart_line_performance "
            "--profiles-dir $DBT_PROFILES_DIR "
            "--project-dir $DBT_PROFILES_DIR "
            "&& dbt test "
            "--select staging.stg_line_status "
            "--profiles-dir $DBT_PROFILES_DIR "
            "--project-dir $DBT_PROFILES_DIR"
        ),
    )
